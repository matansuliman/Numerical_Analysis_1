    ...
    while n <= Nmax:
        ...
        if n >= 2:
            if denominator(p_prev_prev, p_prev, p0) == 0:
                print(f'Method failed (division by 0) after {n} iterations, found: {p_hat}')
                return n-2
            p_hat = hat(p_prev_prev, p_prev, p0)
            print(f' {n-2:2d}     {p_hat:3.10f}    {f(p_hat):3.10f}')
        if n >= 3:
            if np.abs(p_hat - p_hat_prev) < TOL:
                print(f'Method succeeded after {n} iterations, found: {p_hat}')
                return n-2
        n += 1
